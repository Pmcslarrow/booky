...
class UserTower(nn.Module):
    def __init__(self, num_users, num_ages, embedding_dim):
        ...
        self.user_age_embedding = nn.Embedding(
            num_ages, age_embedding_dim, padding_idx=0
        )
...
class ItemTower(nn.Module):
    def __init__(
        ...
        num_authors,
        num_publishers,
        num_year_of_publications,
        embedding_dim,
    ):
        super().__init__()

        book_isbn_embedding_size = 32
        book_author_embedding_size = 32
        book_publisher_embedding_size = 16
        book_year_of_publication_embedding_size = 8

        # Categorical embeddings
        self.book_isbn_embedding = nn.Embedding(
            num_isbns, book_isbn_embedding_size, padding_idx=0
        )
        self.book_author_embedding = nn.Embedding(
            num_authors, book_author_embedding_size, padding_idx=0
        )
        self.book_publisher_embedding = nn.Embedding(
            num_publishers, book_publisher_embedding_size, padding_idx=0
        )
        self.book_year_of_publication_embedding = nn.Embedding(
            num_year_of_publications,
            book_year_of_publication_embedding_size,
            padding_idx=0,
        )

        linear_in = (
            book_isbn_embedding_size
            + book_author_embedding_size
            + book_publisher_embedding_size
            + book_year_of_publication_embedding_size
        )
    ...
    def forward(self, book_isbn, book_author, book_publisher, book_year_of_publication):
        book_isbn_emb = self.book_isbn_embedding(book_isbn)
        ...
        x = torch.cat(
            [book_isbn_emb, book_author_emb, book_publisher_emb, book_year], dim=1
        )
